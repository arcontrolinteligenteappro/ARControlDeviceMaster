...
class DeviceManager {
    ...
    private async setupDeviceTracking() {
        try {
            ...
            tracker.on('end', () => {
                console.log('[DeviceManager] El seguimiento de dispositivos ha finalizado.');
            });
        } catch (err) {
            console.error('[DeviceManager] Error al iniciar el seguimiento de dispositivos:', err);
        }
    }
    ...
    private async addDevice(device: Device) {
        try {
            ...
            console.error(`[DeviceManager] Error al añadir el dispositivo ${device.id}:`, err);
        }
    }
    ...
    private getDeviceType(properties: { [key: string]: string }): DeviceType {
        if (properties['ro.product.model'].toLowerCase().includes('quest')) return 'VR';
    ...
    private sendDeviceList() {
        if (this.window) {
            this.window.webContents.send('devices-updated', Array.from(this.devices.values()));
        }
    }
