...
class DeviceManager {
    ...
    constructor() {
        console.log("DeviceManager initialized. Using direct ADB child_process execution.");
    }
    ...
    private parseAdbDevicesOutput(output: string): Partial<MonitoredDevice>[] {
        ...
        lines.forEach(line => {
            ...
            const product = (parts.find(p => p.startsWith('product:')) || '').split(':')[1] || 'Unknown';
            const device = (parts.find(p => p.startsWith('device:')) || '').split(':')[1] || 'Unknown';
    ...
    public async checkSecurityStatus(deviceId: string): Promise<DeviceSecurityStatus> {
        ...
        try {
            ...
            const { stdout: rootCheck } = await execAsync(`adb -s ${deviceId} shell "su -c id"`);
        ...
        try {
            ...
            const { stdout: knoxCheck } = await execAsync(`adb -s ${deviceId} shell getprop ro.boot.warranty_bit`);
            if (knoxCheck.trim()) {
        ...
        try {
            ...
            const { stdout: mdmCheck } = await execAsync(`adb -s ${deviceId} shell "dumpsys device_policy"`);
    ...
    public async getConnectedDevices(): Promise<MonitoredDevice[]> {
        try {
            ...
            console.error('[DeviceManager] Failed to execute "adb devices -l":', error);
            return [];
        }
    }
}
