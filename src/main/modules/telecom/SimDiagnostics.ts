...
class AdbCommandExecutor {
    public async executeAdbCommand(deviceId: string, command: string): Promise<string> {
        try {
            const { stdout } = await execAsync(`adb -s ${deviceId} ${command}`);
            return stdout.trim();
        } catch (error) {
            console.error(`Error executing ADB command for ${deviceId}: ${command}`, error);
            throw error;
        }
    }
}
...
export class SimDiagnostics extends AdbCommandExecutor {
    ...
    public async releaseNetwork(deviceId: string, unlockCode: string): Promise<void> {
        console.log(`[NCK SIMULATOR] Iniciando desbloqueo de red para ${deviceId}...`);
        
        const isUsbDevice = deviceId.indexOf(':') === -1;
        if (!isUsbDevice) {
            throw new Error('La operación de desbloqueo NCK requiere conexión USB directa.');
        }
        console.log(`[NCK SIMULATOR] Verificación de conexión USB superada.`);

        try {
            // 1. Despertar pantalla
            await this.executeAdbCommand(deviceId, 'shell input keyevent KEYCODE_WAKEUP');
            ...
            console.log(`[NCK SIMULATOR] Proceso de desbloqueo para ${deviceId} finalizado.`);

        } catch (error) {
            console.error(`[NCK SIMULATOR] Falló el proceso de liberación para ${deviceId}.`, error);
            throw error;
        }
    }
}
