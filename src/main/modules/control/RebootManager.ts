...
export class RebootManager {
    ...
    public rebootToRecovery(deviceId: string): Promise<{ stdout: string; stderr: string }> {
        return this.executeCommand(`adb -s ${deviceId} reboot recovery`);
    }

    public rebootToBootloader(deviceId: string): Promise<{ stdout: string; stderr: string }> {
        return this.executeCommand(`adb -s ${deviceId} reboot bootloader`);
    }

    public rebootToDownload(deviceId: string): Promise<{ stdout: string; stderr: string }> {
        return this.executeCommand(`adb -s ${deviceId} reboot download`);
    }

    public rebootToEdl(deviceId: string): Promise<{ stdout: string; stderr: string }> {
