...
class InputEmulation {
  ...
  public tap(deviceId: string, x: number, y: number): void {
    ...
    if (shell) {
      ...
      console.warn(
        `No active shell for device ${deviceId} to send tap command.`,
      );
  ...
  public swipe(
    ...
    if (shell) {
      ...
      console.warn(
        `No active shell for device ${deviceId} to send swipe command.`,
      );
  ...
  public sendText(deviceId: string, text: string): void {
    ...
    if (shell) {
      ...
      console.warn(
        `No active shell for device ${deviceId} to send text command.`,
      );
      // Fallback to single command
      spawn('adb', [
        '-s',
