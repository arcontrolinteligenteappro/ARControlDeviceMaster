...
export class LogcatManager extends EventEmitter {
  ...
  public startStream(deviceId: string) {
    if (this.logcatProcess) {
      console.log(
        `[LogcatManager] Logcat stream for ${deviceId} is already active.`,
      );
      return;
    }
    ...
    console.log(
      `[LogcatManager] Started logcat stream for device: ${deviceId}`,
    );
    ...
    this.logcatProcess.stderr.on('data', (data: Buffer) => {
      console.error(
        `[LogcatManager] Error in logcat stream for ${deviceId}: ${data.toString()}`,
      );
    });

    this.logcatProcess.on('close', (code) => {
      console.log(
        `[LogcatManager] Logcat stream for ${deviceId} closed with code ${code}.`,
      );
    ...
    this.logcatProcess.on('error', (err) => {
      console.error(
        `[LogcatManager] Failed to start logcat for ${deviceId}.`,
        err,
      );
      this.logcatProcess = null;
    });
