...
export class IntentLauncher extends AdbCommandExecutor {
    ...
    public async openDeepLink(deviceId: string, uri: string): Promise<void> {
        ...
        try {
            await this.executeAdbCommand(deviceId, `shell am start -a android.intent.action.VIEW -d "${uri}"`);
            console.log(`[IntentLauncher] Deep Link abierto exitosamente.`);
        } catch (error) {
            console.error(`[IntentLauncher] Falló al abrir el Deep Link: ${uri}`, error);
            throw error;
        }
    }
    ...
    public async openSettings(deviceId: string): Promise<void> {
        ...
        try {
            await this.executeAdbCommand(deviceId, 'shell am start -a android.settings.SETTINGS');
    ...
    public async openFileManager(deviceId: string): Promise<void> {
        ...
        try {
            await this.executeAdbCommand(deviceId, 'shell am start -a android.intent.action.GET_CONTENT -t "*/*"');
            console.log(`[IntentLauncher] Gestor de Archivos abierto exitosamente.`);
        } catch (error) {
            console.error(`[IntentLauncher] Falló al abrir el Gestor de Archivos.`, error);
            throw error;
        }
    }
}
