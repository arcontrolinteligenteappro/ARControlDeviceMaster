...
async function getDeviceTelemetry(id: string): Promise<TelemetryData> {
    ...
    try {
        // Get Battery Info
        const { stdout: batteryOutput } = await execAsync(`adb -s ${id} shell dumpsys battery`);
        ...
        if (statusMatch) {
            const statusMap = ['Unknown', 'Charging', 'Discharging', 'Not charging', 'Full'];
            data.battery.status = statusMap[parseInt(statusMatch[1], 10) -1] || 'Unknown';
        }
    ...
    try {
        ...
        const { stdout: topOutput } = await execAsync(`adb -s ${id} shell top -n 1`);
    ...
    try {
        // Get Temperature
        const { stdout: tempOutput } = await execAsync(`adb -s ${id} shell cat /sys/class/thermal/thermal_zone0/temp`);
